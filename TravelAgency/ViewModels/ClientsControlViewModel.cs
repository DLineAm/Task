...
namespace TravelAgency.ViewModels
{
    public class ClientsControlViewModel : BindableBase
    {
        ...
        public List<Clients> Clients { get => _clients; set => SetProperty(ref _clients, value); }

        private List<Clients> _allClients;
        public List<Clients> AllClients { get => _allClients; set => SetProperty(ref _allClients, value); }

        private Clients _selectedClient;
        public Clients SelectedClient { get => _selectedClient; set => SetProperty(ref _selectedClient, value); }
        ...
        private async void AddClient()
        {
            ...
            App.ShowNotification("Добавление клиента", $"Клиент №{Id} ({result.Surname} {result.Name} {result.Patronymic}) добавлен.");
        }
        ...
        public string NameSearch { get => _nameSearch;
            set
            {
        ...
        public int SelectedGender { get => _selectedGender;
            set
            {
        ...
        private async Task<List<Clients>> GetClients()
        {
            ...
            await using var connection = new SqlConnection(App.GetConString());
            var command = BuilderFactory.NewBuilder<CommandBuilder>()
                .SetCommandText<Clients>(CommandBuilder.CommandType.Get)
                .SetConnection(connection)
                .Build()
                .To<SqlCommand>();
        ...
        private async Task<Clients> OpenClientsDetailsView(HandleType handleType)
        {
            var vm = new ClientDetailsViewModel(SelectedClient, handleType) { Title = handleType != HandleType.Add 
